...
# Usage: curl -fsSL https://app.bumeet.es/downloads/install-macos.sh | bash
#        or:  curl -fsSL https://app.bumeet.es/downloads/install-macos.sh | bash -s -- --token YOUR_API_TOKEN
set -euo pipefail
