...
class FakeBleakClient:
    ...
    def __init__(
        self, address: str, fail_on_connect: bool = False, fail_on_write: bool = False
    ) -> None:
    ...
    async def write_gatt_char(
        self, characteristic_uuid: str, payload: bytes, response: bool = True
    ) -> None:
        if self.fail_on_write:
            raise RuntimeError(f"simulated write failure to {characteristic_uuid}")
        self.writes.append(
            RecordedWrite(
                characteristic_uuid=characteristic_uuid, payload=payload, response=response
            )
        )
    ...
    *, fail_on_connect: bool = False, fail_on_write: bool = False
    ...
    def factory(address: str) -> FakeBleakClient:
        client = FakeBleakClient(
            address, fail_on_connect=fail_on_connect, fail_on_write=fail_on_write
        )
